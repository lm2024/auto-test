<template>
  <div class="execute-detail">
    ...
    <div class="view-toggle">
      <t-space>
        <t-button :theme="viewMode === 'flat' ? 'primary' : 'default'" @click="viewMode = 'flat'" size="small">平铺视图</t-button>
        <t-button :theme="viewMode === 'trace' ? 'primary' : 'default'" @click="viewMode = 'trace'" size="small">分组视图</t-button>
      </t-space>
    </div>
    ...
    <div class="nodes-area" v-if="viewMode === 'flat'">
      <div v-for="log in nodeLogs" :key="log.nodeCode" class="node-log-card" :class="'status-' + log.status.toLowerCase()">
        ...
        <div class="log-actions">
          <t-button size="small" @click="showDetail(log)">查看详情</t-button>
      ...
      <div v-for="(group, gIdx) in traceLogGroups" :key="gIdx" class="trace-group-card">
        <div class="trace-group-header">
          <div class="trace-group-info">
            <t-tag size="small" theme="primary">TraceId</t-tag>
    ...
    <t-dialog :visible="detailVisible" @update:visible="val => detailVisible = val" header="节点详情" :width="700">
      <div v-if="currentLog">
        ...
        <MonacoEditor :value="formatJson(currentLog.requestHeaders)" language="json" :height="120" :readonly="true" @update:modelValue="currentLog.requestHeaders = $event" />
        <t-button size="small" @click="copyText(currentLog.requestHeaders)">复制</t-button>

        <div class="section-title">请求体</div>
        <MonacoEditor :value="formatJson(currentLog.requestBody)" language="json" :height="200" :readonly="true" @update:modelValue="currentLog.requestBody = $event" />
        <t-button size="small" @click="copyText(currentLog.requestBody)">复制</t-button>

        <div class="section-title">响应体</div>
        <MonacoEditor :value="formatJson(currentLog.responseBody)" language="json" :height="200" :readonly="true" @update:modelValue="currentLog.responseBody = $event" />
        <t-button size="small" @click="copyText(currentLog.responseBody)">复制</t-button>

        <div v-if="currentLog.errorMessage" class="section-title" style="color:#f56c6c">错误信息</div>
        <MonacoEditor v-if="currentLog.errorMessage" :value="currentLog.errorMessage" language="plaintext" :height="100" :readonly="true" />
      </div>
    </t-dialog>

    <t-dialog :visible="aiVisible" @update:visible="val => aiVisible = val" header="AI分析结果" :width="650" :close-on-overlay-click="false">
      ...
      <div v-else-if="aiResult">
        ...
        <div class="analysis-footer">
          <t-button size="small" @click="copyText(JSON.stringify(aiResult, null, 2))">复制全部</t-button>
        </div>
      </div>
  ...
  methods: {
    ...
    async analyzeFailure(log) {
      ...
      try {
        ...
        this.$message.error('分析失败: ' + (e.response && e.response.data && e.response.data.message ? e.response.data.message : e.message))
      } finally {
    ...
    statusType(s) {
      return ({ RUNNING: 'warning', SUCCESS: 'success', FAILED: 'danger', SKIPPED: 'info' }[s] || 'info')
    },
    statusText(s) {
      return ({ RUNNING: '运行中', SUCCESS: '成功', FAILED: '失败', SKIPPED: '跳过', PENDING: '待执行' }[s] || s)
    }
  },
  mounted() {
...
.toolbar ::v-deep(.t-button) {
  border-radius: 10px;
  font-weight: 500;
  transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1);
}

.toolbar ::v-deep(.t-button--theme-primary) {
...
::v-deep(.t-descriptions) {
  border-radius: 12px;
  overflow: hidden;
}

::v-deep(.t-descriptions__label) {
